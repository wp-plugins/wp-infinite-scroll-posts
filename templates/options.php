...
<h3 class="title"><?php _e("Front-end Setting","wp-easy-scroll-posts") ?></h3>
 ...
 <table class="form-table admintbl">
	...
	<tr>
		...
		<td>
			<div id="<?php echo user_can_richedit() ? 'postdivrich' : 'postdiv'; ?>" class="postarea">
    			...
    			<textarea id="wp_easy_scroll_posts[loading][msgText]" class="wp-editor-area" name="wp_easy_scroll_posts[loading][msgText]" cols="60" rows="5"><?php echo $finishedMsg = esc_attr( $this->parent->options->loading["msgText"] ); ?></textarea>
		...
		<td>
    	  ...
    	  <?php $align = esc_attr( $this->parent->options->loading["align"] );
			...
			<tr>
		<th>
			<label><?php _e( 'Loading Speed', 'wp-easy-scroll-posts' ); ?>	</label>	
		</th>
		<td>
    		<select name="wp_easy_scroll_posts[loading][speed]" class="wp_easy_scroll_posts[loading][speed]" id="wp_easy_scroll_posts[loading][speed]">
    	  <?php  $speed = esc_attr( $this->parent->options->loading["speed"] );
    		$slow = "";
    		$fast = "";
    		 if ($speed=='slow'){
    		 	 $slow = 'selected="selected"';
    		 	} else if($speed=='fast'){
    		 		$fast = 'selected="selected"';
    		 		} 
    		 
				echo '<option value="fast" '.$fast.' >fast</option>
						<option value="slow" '.$slow.' >slow</option>';
				?>
			</select>
			<br />
			<span class="description"><?php _e( 'Loading Speed options.', 'wp-easy-scroll-posts' ); ?></span>
	...
	<tr>
		...
		<td>
			<div id="<?php echo user_can_richedit() ? 'postdivrich' : 'postdiv'; ?>" class="postarea">
    			...
    			<textarea id="wp_easy_scroll_posts[loading][finishedMsg]" class="wp-editor-area" name="wp_easy_scroll_posts[loading][finishedMsg]" cols="60" rows="5"><?php echo $finishedMsg = esc_attr( $this->parent->options->loading["finishedMsg"] ); ?></textarea>
	...
	<tr>
		<th>
			<label><?php _e( 'Show MaxPage', 'wp-easy-scroll-posts' ); ?>	</label>	
		</th>
		<td>
		<?php $maxPage = esc_attr( $this->parent->options->maxPage );
		if(($maxPage == '') || ($maxPage == 'Null')){
			$maxPage = 'undefined';
			} ?>
			<input type="text" name="wp_easy_scroll_posts[maxPage]" id="wp_easy_scroll_posts[maxPage]" value="<?php echo $maxPage; ?>" class="regular-text" /><br />
			<span class="description"><?php _e( 'Set limit for show maximum number of pages to scroll on current page.<br/>
Note: if you want to show all post pages please use maxPage value - "undefined" ', 'wp-easy-scroll-posts' ); ?></span>
		</td>
	</tr>
</table>
<p class="submit">
