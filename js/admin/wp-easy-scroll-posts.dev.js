...
//alert('hello');
	...
	$('#upload_image_button').click(function() {
		formfield = $('#upload_image').attr('name');
	 	tb_show('', 'media-upload.php?type=image&tab=library&wp_easy_scroll_posts=true&TB_iframe=true');
	 	return false;
	});
	...
	$( '#the-list .save-link a' ).live( 'click', function(event){
		...
		$.ajax( {
			url: ajaxurl + '?action=wp-easy-scroll-posts-edit-preset', 
			type: 'POST',
	...
	$( '.delete' ).live( 'click', function( event ) {
		...
		if ( !confirm( wp_easy_scroll_posts.confirm.replace( '%s', theme ) ) )
			return false;
		
		var tr = $(this).parent().parent().parent();
		
		$.ajax( {
			url: ajaxurl + '?action=wp-easy-scroll-posts-delete-preset&theme=' + theme, 
			type: 'POST',
	...
	$('#use_default').click( function(event) {
		event.preventDefault();
		$('#wp_easy_scroll_posts_form').append( '<input type="hidden" name="reset_default_image" value="1" />' ).submit();
		return false;
	});
